...
function setup(options, cb) {
  ...
  createDir(path)
  createDir(path + '/dats')
  createDir(path + '/follows')
  createDir(path + '/relationships')
  ...
  createPgpKey(pgpOptions, (pgpKeys) => {
    ...
    createPublicMetadat(datDir, (dat) => {
      ...
      const pubPath = path + '/' + name 
      const pubListPath = pubPath + '/' + 'pub.json'
      const pub = { dats: [] }
      ...
      fs.writeFileSync(pubPath + '/public.key', pgpKeys.publicKeyArmored)
