...
module.exports = function checkHandshake (userA, key, cb) {
  follow(userA, key, (userA, userB) => {
    const hsFilePath = userA.dirs.follows + '/' + userB.name + '-' + userB.id + '/handshakes/' + userA.id
    const privateKey = userA.privKey
    const publicKey = userB.pubKey
    ...
    if (!result) throw new Error('Could not decrypt', cipher)
