...
function registerPass (pass, path, callback) {
  ...
  sodium.mlock(salt)
  fs.writeFile(path + '/salt', salt, (err) => {
    if (err) throw err
    sodium.crypto_pwhash_str_async(output, password, opslimit, memlimit, (err) => {
      if (err) throw err
      sodium.mlock(output)
      fs.writeFile(path + '/pass_hash', output, (err) => {
        if (err) throw err
      })
      callback(salt)
    })
...
function derivePrivKey (pass, salt, callback) {
  ...
  sodium.crypto_pwhash_async(output, password, salt, opslimit, memlimit, algorithm, (err) => {
    if (err) throw err
    sodium.mlock(output)
    callback(output)
  })
...
function verifyPass (hash, pass, callback) {
  const password = new Buffer(pass)
  sodium.crypto_pwhash_str_verify_async(hash, password, (err, bool) => {
    if (err) throw err
    callback(bool)
  })
}
